...
pub(super) fn derive_packet(item: TokenStream) -> Result<TokenStream> {
    ...
    let packet_id: Expr = match packet_attr.id {
        ...
        None => match syn::parse_str::<Ident>(&name_str.to_shouty_snake_case()) {
            Ok(ident) => parse_quote!(::protocol_lib::packet_id::#ident),
            Err(_) => {
    ...
    let side = if let Some(side_attr) = packet_attr.side {
        ...
        parse_quote!(::protocol_lib::packets::types::PacketSide::Clientbound)
    } else if name_str.to_lowercase().ends_with("c2s") {
        parse_quote!(::protocol_lib::packets::types::PacketSide::Serverbound)
    } else {
        return Err(Error::new(
            packet_attr.span,
    ...
    let state = packet_attr
        .state
        .unwrap_or_else(|| parse_quote!(::protocol_lib::types::PacketState::Play));

    Ok(quote! {
        impl #impl_generics ::protocol_lib::__private::Packet for #name #ty_generics
        #where_clause
        {
            const ID: i32 = #packet_id;
            const NAME: &'static str = #name_str;
            const SIDE: ::protocol_lib::packets::types::PacketSide = #side;
            const STATE: ::protocol_lib::packets::types::PacketState = #state;
        }
    })
}
