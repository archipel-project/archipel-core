...
use crate::utils::{add_trait_bounds, decode_split_for_impl, pair_variants_with_discriminants};
...
pub(super) fn derive_decode(item: TokenStream) -> Result<TokenStream> {
    ...
    match input.data {
        Data::Struct(struct_) => {
            ...
            add_trait_bounds(
                &mut input.generics,
                quote!(::protocol_lib::__private::Decode<#lifetime>),
            );
            ...
            Ok(quote! {
                ...
                impl #impl_generics ::protocol_lib::__private::Decode<#lifetime> for #input_name #ty_generics
                #where_clause
                {
                    fn decode(_r: &mut &#lifetime [u8]) -> ::protocol_lib::__private::Result<Self> {
                        use ::protocol_lib::__private::{Decode, Context, ensure};
        ...
        Data::Enum(enum_) => {
            ...
            Ok(quote! {
                ...
                {
                    fn decode(_r: &mut &#lifetime [u8]) -> ::protocol_lib::__private::Result<Self> {
                        use ::protocol_lib::__private::{Decode, Context, VarInt, bail};
